...
const checkSecret = (minStrength, passwordString, zxcvbnOptions) => {
  ...
  const guesses = Math.floor(strongEnough.guesses_log10).toFixed()
  if (warning) bold(true, chalk.red('--- special alert: %s'), warning)
  bold(false, 'Ask questions like: who/what will that secret defend against?')
  bold(true, '--- security broken, assuming est. ~1e%s attempts to crack:', guesses)
  ...
  bold(true, chalk.green('Analysis by %s took: %sms'), 'zxcvbn', strongEnough.calc_time)
}
...
program // unstable
  ...
  .option(
    ...
    '--model',
    'adversary score (integer 0-5, where 0 = accept anything, and 5 = never strong enough; 2 = professional)',
    2,
  )
  .action(async (password, options, command) => {
    ...
    */
    ...
    const isExtraParanoid = JSON.parse(process.env.EXTRA_PARANOID || 'false')
    const minStrength = parseInt(options.model, 10) || (isExtraParanoid ? 4 : 2)
    console.log(minStrength, 'options:', options)
    console.error(chalk.blue('--- checking password strength...'))
    if (checkSecret(minStrength, s, zxcvbnWordList)) {
      console.log(chalk.green('--- OK'))
    } else {
      console.error(chalk.red('--- KO'))
    }
  })
