 ...
 * - bootstrap+secret (create an account)
...
const login = async (env = process.env, useMode = 'tenant') => {
  ...
  const prefix = env.SOCOTRA_TENANT || `${env.ADMIN_USERNAME}-configeditor`
  const suffix = env.SOCOTRA_DOMAIN || SANDBOX_DOMAIN_NAME // after .co.
  const hostname = env.TENANT_HOSTNAME || `${prefix}.co.${suffix}`
  const url = env.API_URL || `https://api.${suffix}` // base API

  // by default: login(..., !!truthy) is tenant mode, else: admin mode
  if (typeof useMode !== 'string') {
    //let tenant = useMode || useMode.hostname
    useMode = useMode ? 'tenant' : 'admin'
  }
  // TODO: support some JWT and SSO "bootstrap" magic (for org admins)
  if (useMode.startsWith('bootstrap+')) {
    throw new Error('JWT+SSO login is not yet supported (by this tool)')
  }
  ...
  const questions2 = [
    ...
    },
    {
      ...
      name: 'key',
      type: 'password',
      when: useMode === 'bootstrap+secret',
    },
    {
      message: 'Magic bootstrap (will open browser for SSO, where needed):',
      name: 'sso',
      type: 'password',
      when: useMode === 'bootstrap+magic',
    },
  ]
